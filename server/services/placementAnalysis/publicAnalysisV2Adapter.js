...
const { PUBLIC_ANALYSIS_VERSION, findForbiddenPublicKeys } = require("../../contracts/publicAnalysisV2");

const labelFor = (key) => READINESS_LABELS.bands.find((band) => band.key === key)?.label || key;
const publicLanguageInsight = (item) => ({ id: item.insightId, text: item.text });
...
const createPublicAnalysisV2 = (snapshot, language) => {
  ...
  const response = {
    ...
    readiness: {
      ...
      explanation: snapshot.readiness.explanation
    },
  ...
  if (forbidden.length) throw new Error(`Public analysis V2 contains forbidden fields: ${forbidden.join("; ")}`);
  return response;
};
