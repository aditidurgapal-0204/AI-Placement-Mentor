...
const generateRoadmapLanguage = async (prompt, { requestId = "untracked" } = {}) => {
  const model = getGeminiClient().getGenerativeModel({
    model: "gemini-2.5-flash",
    generationConfig: {
      temperature: 0,
      ...
      responseSchema: {
        ...
        properties: {
          roadmap: { type: "array", items: { type: "object", properties: {
            month: { type: "integer" }, title: { type: "string" }, focus: { type: "string" },
            topics: { type: "array", items: { type: "string" } }, goals: { type: "array", items: { type: "string" } }
          }, required: ["month", "title", "focus", "topics", "goals"] } }
        }, required: ["roadmap"]
      },
      thinkingConfig: { thinkingBudget: 0 }
    }
  ...
  const response = await result.response;
  const finishReason = response.candidates?.[0]?.finishReason || "UNKNOWN";
  console.info("Gemini roadmap metadata", { requestId, finishReason, totalTokenCount: response.usageMetadata?.totalTokenCount });
  if (finishReason !== "STOP") throw new Error(`Gemini roadmap ended with finish reason ${finishReason}.`);
  return response.text();
};

const generatePublicResumeSummary = async (prompt, { requestId = "untracked" } = {}) => {
  const model = getGeminiClient().getGenerativeModel({
    model: "gemini-2.5-flash",
    generationConfig: {
      temperature: 0,
  ...
  const response = await result.response;
  const finishReason = response.candidates?.[0]?.finishReason || "UNKNOWN";
  console.info("Gemini public resume summary metadata", { requestId, finishReason, totalTokenCount: response.usageMetadata?.totalTokenCount });
  if (finishReason !== "STOP") throw new Error(`Gemini resume summary ended with finish reason ${finishReason}.`);
  return response.text();
};

const generateMockInterviewTurn = async (prompt, { requestId = "untracked" } = {}) => {
  const ratingProperties = Object.fromEntries([
    "Answer Relevance", "Clarity", "Response Structure", "Professionalism", "Communication Quality", "Assertiveness",
    "Technical Knowledge", "Concept Clarity", "Answer Accuracy", "Problem Solving", "Explanation Quality", "Resume Knowledge"
      ...
      responseSchema: {
        ...
        properties: {
          evaluation: { type: "object", properties: {
            ratings: { type: "object", properties: ratingProperties }
          }, required: ["ratings"] },
          nextQuestion: { type: "object", properties: {
            question: { type: "string" }, category: { type: "string" }, topic: { type: "string" }, isFollowUp: { type: "boolean" }
          }, required: ["question", "category", "topic", "isFollowUp"] }
  ...
  const response = await result.response;
  const finishReason = response.candidates?.[0]?.finishReason || "UNKNOWN";
  console.info("Gemini mock interview turn metadata", { requestId, finishReason, totalTokenCount: response.usageMetadata?.totalTokenCount });
  if (finishReason !== "STOP") throw new Error(`Gemini interview turn ended with finish reason ${finishReason}.`);
  return response.text();
};
...
module.exports = { analyzePlacementProfile, generatePresentationLanguage, generateRoadmapLanguage, generatePublicResumeSummary, generateMockInterviewTurn, testConnection };
