...
const textFor = (item) => `${item.title || item.type} is supported by the verified analysis for this profile.`;
const groundedOutput = (dto) => ({
  analysisId: dto.analysisId,
  diagnosis: groundedDiagnosis,
  strengths: dto.strengths.map((item) => ({ insightId: item.id, text: textFor(item) })),
  scoreBlockers: dto.scoreBlockers.map((item) => ({ insightId: item.id, text: `Improve ${item.title || item.type} for the selected role.` })),
  careerRisks: dto.careerRisks.map((item) => ({ insightId: item.id, text: `Improve ${item.title || item.type} so recruiters can evaluate the profile more clearly.` })),
