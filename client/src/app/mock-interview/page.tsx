...
export default function MockInterviewPage() {
  ...
  const startInterview = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(apiUrl("/api/mock-interview/start"), { method: "POST", body: form, headers: { "X-Request-ID": crypto.randomUUID() } });
  ...
  const submitAnswer = async (event: FormEvent) => {
    ...
    setPending(true); setError(null);
    try {
      const response = await fetch(apiUrl(`/api/mock-interview/${encodeURIComponent(interview.interviewId)}/answer`), { method: "POST", headers: { "Content-Type": "application/json", "X-Request-ID": crypto.randomUUID() }, body: JSON.stringify({ answer: answer.trim() }) });
